...
// export const baseURL = "http://localhost:2118/eimpact/api/v1/organization";
export const baseURL = "https://anorahr-backend.anorateck.com/";
...
axiosClient.interceptors.request.use((config) => {
  const accessToken = getDecryptedToken("EimpactAccessToken");
...
axiosClient.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (
      ...
      const refreshToken = getDecryptedToken("EimpactRefreshToken");
      const accessToken = getDecryptedToken("EimpactAccessToken");
      ...
      try {
        const response = await axios.post("/auth/get-access-token", {
          refreshToken,
        });
    ...
    if (status === 403 && !originalRequest?._retry) {
      ...
      const refreshToken = getDecryptedToken("NCEXrefreshToken");
      const accessToken = getDecryptedToken("NCEXaccessToken");
